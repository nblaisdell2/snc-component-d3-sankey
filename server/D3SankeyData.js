 ...
 * the x-2114311-sankey-chart-uic component's "Data · Graph data" property:
 *
 *   {
...
D3SankeyData.prototype = {
	...
	_build: function (rows, cfg, dupMetric) {
		...
		var srcLabel = function (v) {
			return prefix ? "From: " + v : v;
		};
		var tgtLabel = function (v) {
			return prefix ? "To: " + v : v;
		};
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (s === t) {
				continue;
			} // d3-sankey can't lay out a self-loop
			addNode(s);
			addNode(t);
			var key = s + "" + t;
