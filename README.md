...
- **Component tag:** `x-1295779-sankey-chart-uic`
- **Scope:** `x_1295779_sankey_0`
...
> vendor prefix (`x_1295779`) and architecture, but its **data shape is
...
```
src/x-1295779-sankey-chart-uic/
...
(category *Primitives*). Bind `data` to a data resource (or leave it empty to show
...
- **`nodes`** — the stages/buckets. Each is `{ name, color? }`; the optional
  per-node `color` is used when *Colors · Use node colors* is on.
- **`links`** — the flows. Each is `{ source, target, value }`. The ribbon's
  thickness is proportional to `value`.

### `source` / `target` may be a node NAME *or* a node INDEX
...
a friendly empty-state message (*"Cannot render flow (check for circular
links)"*) instead of crashing. An empty graph (no nodes/links) shows *"No data to
display"*.
...
*Data · Graph data*. All transform logic lives in a reusable **Script Include**
...
- **Bind:** *Data · Graph data* → `@data.d3_sankey_data.output` (use your
  resource's name).
...
Run `server/sanity-test.background.js` in *Scripts - Background* (Global scope) to
...
| Color palette | `colorPalette` | 6-color set | JSON array cycled across nodes when *Use node colors* is off and scheme is Custom. |
