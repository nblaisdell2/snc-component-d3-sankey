...
const getContainer = (host) =>
	host && host.shadowRoot
		? host.shadowRoot.querySelector('.sc-root') || host.shadowRoot.querySelector('div')
		: null;
...
const hasGraph = (g) => g && typeof g === 'object' && Array.isArray(g.nodes) && g.nodes.length > 0;
...
const render = ({ host, properties, dispatch }) => {
	...
	try {
		...
		host._scWidth = container.getBoundingClientRect().width || container.clientWidth || 0;
	} catch (e) {
		...
		if (typeof console !== 'undefined') console.error('[sankey-chart] render failed', e);
	}
};

createCustomElement('x-1295779-sankey-chart-uic', {
	...
	properties: {
		...
		tooltipTemplate: { default: '<strong>{name}</strong><br/>{swatch}Total: {formattedValue}' },
